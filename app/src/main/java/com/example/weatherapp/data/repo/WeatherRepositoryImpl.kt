...
import com.example.weatherapp.domain.repo.mapper.toCityLocationDto
import com.example.weatherapp.domain.repo.mapper.toWeatherResponse
...
class WeatherRepositoryImpl(
    ...
    override fun getWeatherByLocation(cityLocation: CityLocation): Flow<WeatherResponse> {
        return flow {
            wrapCallWithTry {
                emit(
                    remoteDataSource.getWeatherByLocation(cityLocation.toCityLocationDto())
                        .toWeatherResponse()
                )
            }
        }
